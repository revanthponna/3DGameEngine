...
struct sDataRequiredToRenderAFrame
{
	...
	uint16_t numRenderables = 0;

	void CleanUp()
	{
		for (uint16_t i = 0; i < numRenderables; i++)
		{
...
// Creating a new render context
eae6320::Graphics::RenderContext newRenderContext;
...
void eae6320::Graphics::RenderFrame()
{
	...
	// Clearing buffers from the RenderContext class
	// Setting the background color according to the user-set color
	newRenderContext.ClearBuffers(s_dataBeingRenderedByRenderThread->hexCode);
	...
	// Bind effect and render mesh one after the other
	Render();

	newRenderContext.SwapBuffer();
...
eae6320::cResult eae6320::Graphics::Initialize(const sInitializationParameters& i_initializationParameters)
{
	...
	// Initialize the views from the RenderContext class
	result = newRenderContext.InitializeViewsCheck(result, i_initializationParameters);
...
eae6320::cResult eae6320::Graphics::CleanUp()
{
	...
	// Cleaning Up Views from the RenderContext class
	newRenderContext.CleanUpViews();
...
void eae6320::Graphics::SetRenderableObjects(ObjectRenderData pRenderableData[], uint16_t pNumRenderables)
{
	...
	for (uint16_t i = 0; i < pNumRenderables; i++)
	{
...
{
	for (uint16_t i = 0; i < s_dataBeingRenderedByRenderThread->numRenderables; i++)
	{
		// Updating the draw constant buffer
		{
			// Copying the data from the system memory that the application owns to GPU memory
			s_constantBuffer_draw.Update(&s_dataBeingRenderedByRenderThread->Renderables[i].drawData);
		}

		s_dataBeingRenderedByRenderThread->Renderables[i].effect->BindShadingData();
