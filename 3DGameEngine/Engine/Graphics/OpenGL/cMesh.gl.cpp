...
eae6320::cResult eae6320::Graphics::Mesh::InitializeGeometry(GeometryData& pInitData) 
{
	...
	// Assign the data to the index buffer
	{
		...
		EAE6320_ASSERT(bufferSize <= std::numeric_limits<GLsizeiptr>::max());
		glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(bufferSize), reinterpret_cast<GLvoid*>(pInitData.indexData),
			// In our class we won't ever read from the buffer
			GL_STATIC_DRAW);
		const auto errorCode = glGetError();
		if (errorCode != GL_NO_ERROR)
...
void eae6320::Graphics::Mesh::DrawGeometry() {
	{
		...
		// Render geometry from the bound index buffer
		{
			...
			// const unsigned int indexCountToRender = indexCount;	// Total number of indices to render (for two triangles, it's 6 indices)
