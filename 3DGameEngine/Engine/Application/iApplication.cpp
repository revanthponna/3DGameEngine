...
int eae6320::Application::iApplication::ParseEntryPointParametersAndRun( const sEntryPointParameters& i_entryPointParameters )
{
	...
	// (this also starts the application loop on a separate thread)
	{
		...
		if ( result )
		{
			Logging::OutputMessage( "Revanth Ponna's EAE6320 Game was successfully initialized" );
		}
		else
		{
			exitCode = EXIT_FAILURE;
			EAE6320_ASSERT( false );
			Logging::OutputError( "Revanth Ponna's EAE6320 Game initialization failed!" );
			UserOutput::Print( "Revanth Ponna's EAE6320 Game Initialization failed! (Check the log file for details.) This program will now exit." );
			return exitCode;
		}
	}
...
eae6320::cResult eae6320::Application::iApplication::CleanUp_all()
{
	...
	// Clean up the derived application
	{
		...
		{
			EAE6320_ASSERTF( false, "Revanth Ponna's EAE6320 Game couldn't be cleaned up" );
			if ( result )
			{
		...
		{
			Logging::OutputMessage("Revanth Ponna's EAE6320 Game cleaned up successfully");
		}
	}
